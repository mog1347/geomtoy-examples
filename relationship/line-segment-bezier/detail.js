...
const main = () => {
  ...
  function lineSegmentBind([e1, e2]) {
  ...
  const ls1 = g.LineSegment().bind([[pC, "any"], [pD, "any"]], lineSegmentBind).on("any", function () {
  ...
  const ls2 = g.LineSegment().bind([[pE, "any"], [pF, "any"]], lineSegmentBind).on("any", function () {
    this.point1 = l.getClosestPointFrom(this.point1Coordinates);
    this.point2 = l.getClosestPointFrom(this.point2Coordinates);
  ...
  const group = g.Group().bind([[ls1, "any"], [ls2, "any"]], function ([e1, e2]) {
  ...
  const overlap = new ViewElement(group, false, {
    stroke: color("pink"),
    strokeWidth: 6
  }, hoverStyle, activeStyle);
  view.add(new ViewElement(pC, true, ...interactableStyles.point)).add(new ViewElement(pD, true, ...interactableStyles.point)).add(new ViewElement(pE, true, ...interactableStyles.point)).add(new ViewElement(pF, true, ...interactableStyles.point)).add(new ViewElement(ls1, true, {
    stroke: color("orange"),
    strokeWidth: 4
  }, hoverStyle, activeStyle)).add(new ViewElement(ls2, true, {
    stroke: color("teal"),
    strokeWidth: 4
  }, hoverStyle, activeStyle)).add(new ViewElement(l, false, {
    stroke: color("gray"),
    strokeWidth: 2
  }, hoverStyle, activeStyle)).add(overlap);
