...
import { j as Relationship, U as Utility, Q as QuadraticBezier, B as Bezier } from '../../js/geomtoy.js';
...
tpl.title("Quadratic bezier-bezier relationship");
const rs = new Relationship();
{
  tpl.addSection("No Equal");
}
{
  tpl.addSection("Separate");
  {
    const card = tpl.addCard({
      title: "common bezier equal",
      canvasId: Utility.uuid()
    });
    const quadraticBezier = new QuadraticBezier([0, 0], [10, 20], [10, 5]);
    const bezier = new Bezier([0, 0], [10, 20], [10, 5], [-10, -10]);
    trileanResult(card, quadraticBezier, bezier, rs.separate(quadraticBezier, bezier));
  }
}
{
  tpl.addSection("Intersect");
  {
    ...
    const quadraticBezier = new QuadraticBezier([0, 0], [10, 20], [10, 5]);
    const bezier = new Bezier([0, 0], [10, 20], [10, 5], [-10, -10]);
    arrayResult(card, quadraticBezier, bezier, rs.intersect(quadraticBezier, bezier));
  }
  {
    const card = tpl.addCard({
  ...
  {
    const card = tpl.addCard({
      title: "degenerate to line segment and line segment intersect",
      canvasId: Utility.uuid()
    });
  ...
  {
    const card = tpl.addCard({
      title: "degenerate to line segment and quadratic bezier intersect",
      canvasId: Utility.uuid()
    });
    const bezier1 = new Bezier([-60, -60], [60, 60], [-20, -20], [20, 20]);
  ...
  {
    const card = tpl.addCard({
      title: "degenerate to quadratic bezier and quadratic bezier intersect",
      canvasId: Utility.uuid()
    });
  ...
  {
    const card = tpl.addCard({
      title: "on same trajectory intersect and self-intersecting ",
      canvasId: Utility.uuid()
    });
  ...
  {
    const card = tpl.addCard({
      title: "triple lines bezier and triple lines bezier intersect",
      canvasId: Utility.uuid()
    });
  ...
  {
    const card = tpl.addCard({
      title: "common bezier and triple lines bezier intersect",
      canvasId: Utility.uuid()
    });
    const bezier1 = new Bezier([5, -5], [5, -11], [-16, 7], [25, 13]);
    const bezier2 = new Bezier([1, 2], [5, 10], [-10, -20], [0, 0]);
...
}
{
  ...
  {
    const card = tpl.addCard({
      title: "not at end contact = touch",
      canvasId: Utility.uuid()
    });
...
}
{
  ...
  {
    const card = tpl.addCard({
      title: "block and self-intersecting",
      canvasId: Utility.uuid()
    });
  ...
  {
    const card = tpl.addCard({
      title: "blocked by and self-intersecting",
      canvasId: Utility.uuid()
    });
    const bezier = new Bezier([5, -5], [5, -5], [-16, 7], [25, 13]);
...
}
{
  ...
  {
    const card = tpl.addCard({
      title: "1 point coincide and self-intersecting",
      canvasId: Utility.uuid()
    });
  ...
  {
    const card = tpl.addCard({
      title: "equal coincide and self-intersecting",
      canvasId: Utility.uuid()
    });
    const bezier1 = new Bezier([-18, 3], [-18, 3], [-36, 29], [6, -7]);
  ...
  {
    const card = tpl.addCard({
      title: "segment coincide and self-intersecting",
      canvasId: Utility.uuid()
    });
    const bezier1 = new Bezier([-16, 22], [9, 8], [34, -19], [-23, -16]);
